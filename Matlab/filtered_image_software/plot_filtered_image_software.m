...
if (numel(file) > 1)
    for i=1:numel(file)            
        ...
        filtered_image_soft(1,i) = {uint8(reshape(filtered_image_vector(:,i), [dim(1) dim(1)]))'}; %utilizzare 'uint8' al posto di 'double' per saturazione
%         imwrite(filtered_image_hard{i}, [file{i},'.tif']);
    ...
    subplot(2,3,2)
    imshow(filtered_image_soft{1});
    title (['{\color{black}Laplaciano-Gaussiano1 MATLAB}']); 
    subplot(2,3,3)
    imshow(filtered_image_soft{2});
    title (['{\color{black}Laplaciano-Gaussiano2 MATLAB}']);     
    subplot(2,3,4)
    imshow(filtered_image_soft{3});
    title (['{\color{black}Laplaciano1 MATLAB}']); 
    subplot(2,3,5)
    imshow(filtered_image_soft{4});
    title (['{\color{black}Laplaciano2 MATLAB}']); 
    subplot(2,3,6)
    imshow(filtered_image_soft{5});
    title (['{\color{black}Laplaciano3 MATLAB}']); 
    ...
    filtered_image_soft = {double(reshape(filtered_image_vector, [dim(1) dim(1)]))'};
    figure(1)
    imshow(original_image);
    figure(2)
    imshow(filtered_image_soft{1});
